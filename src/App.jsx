    ...
    <div style={{ minHeight:"100vh", background:C.sidebar, display:"flex", flexDirection:"column", fontFamily:FONT, animation:"fadeIn .2s ease" }}>
      ...
      <nav style={{ flex:1, padding:"16px", overflowY:"auto" }}>
        ...
        {MOB_NAV.map(({ id, label, icon:Icon, desc }) => {
          ...
          return (
            <button key={id} onClick={() => onNav(id)} style={{ width:"100%", display:"flex", alignItems:"center", gap:14, padding:"14px 16px", borderRadius:14, border:"none", background:active?"rgba(37,99,235,0.3)":"rgba(255,255,255,0.03)", cursor:"pointer", marginBottom:6, textAlign:"left", borderLeft:active?`3px solid ${C.accent}`:"3px solid transparent", transition:"all .15s" }}>
...
export default function App() {
  ...
  function goHome() { setShowSidebar(true); setSel(null); }
  ...
  function logout() { setUser(null); setSel(null); setPage("dashboard"); setData(SAMPLE_DATA); setShowSidebar(true); }
  ...
  if (isMobile) {
    ...
    return (
      <div style={{ minHeight:"100vh", background:C.bg, fontFamily:FONT }}>
        ...
        {page === "munkalapok" && sel ? (
          <>
            <TopBar title={sel.id} user={user} driveStatus={drive} onBack={() => setSel(null)} backLabel="Munkalapok" isMobile />
            <MunkalapDetail m={sel} data={data} />
          </>
        ) : (
          <>
            <TopBar title={PAGE_TITLES[page]} user={user} driveStatus={drive} onBack={goHome} backLabel="Főmenü" isMobile />
