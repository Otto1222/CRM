  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.55)", zIndex:200, display:"flex", alignItems:"center", justifyContent:"center", padding:24 }}>
      <div style={{ background:"#fff", borderRadius:20, padding:"32px 28px", width:"100%", maxWidth:420, boxShadow:"0 20px 60px rgba(0,0,0,.3)", fontFamily:"'DM Sans',sans-serif" }}>
        ...
        <h2 style={{ textAlign:"center", fontSize:20, fontWeight:800, color:"#0F172A", marginBottom:10 }}>Munkalap törlése</h2>
        <p style={{ textAlign:"center", fontSize:14, color:"#64748B", marginBottom:6 }}>Biztosan törölni szeretnéd ezt a munkalapot?</p>
        ...
        <p style={{ textAlign:"center", fontSize:13, color:"#DC2626", marginBottom:24, fontWeight:600 }}>⚠️ Ez a művelet nem visszavonható!</p>
        <div style={{ display:"flex", gap:12 }}>
          <button onClick={onCancel} style={{ flex:1, padding:"13px", borderRadius:12, border:"1.5px solid #E2E8F0", background:"#fff", fontWeight:700, fontSize:15, cursor:"pointer", color:"#475569", fontFamily:"'DM Sans',sans-serif" }}>Mégsem</button>
          <button onClick={onConfirm} style={{ flex:1, padding:"13px", borderRadius:12, border:"none", background:"#DC2626", color:"#fff", fontWeight:700, fontSize:15, cursor:"pointer", fontFamily:"'DM Sans',sans-serif" }}>Igen, törlöm</button>
        </div>
      </div>
    </div>
  );
...
// ─── FIX: setData és setPage prop PageContent-nek ────────────
function PageContent({ page, sel, setSel, data, setData, setPage, user, onNewMunkalap, onDelete }) {
  const role = user?.role;
  if (page === "munkalapok" && sel) return (
    <MunkalapDetail m={sel} data={data} userRole={role}
      onBack={(refresh) => {
        if (refresh) { const f=loadLocal("munkalapok"); if(f) setData(p=>({...p,munkalapok:f})); }
        setTimeout(() => setSel(null), 50);
      }}
      onDelete={onDelete}
      onRefresh={(updates) => {
        const f=loadLocal("munkalapok"); if(f) setData(p=>({...p,munkalapok:f}));
  ...
  if (page==="projektek")     return <ProjektekPage data={data} currentUser={user} onNavigateMunkalap={(m)=>{setPage("munkalapok");setSel(m);}}/>;
...
export default function App() {
  ...
  useEffect(() => {
    function handleDbUpdate(e) {
      ...
      if (col==="munkalapok"||col==="all") {
        ...
        if(f){ setData(p=>({...p,munkalapok:f})); setSel(p=>p?f.find(m=>m.id===p.id)||p:p); }
      }
  ...
  useEffect(() => {
    ...
    (async () => {
      if (driveAvailable()) { const ok=await drivePing(); setDriveOnline(ok); if(ok) setDrive("ok"); }
      setDrive("saving");
      const [ml, uk] = await Promise.all([driveLoad("munkalapok"), driveLoad("ugyfelek")]);
      const next = { ...data };
      let updated = false;
      if (ml?.munkalapok) { next.munkalapok=ml.munkalapok; updated=true; saveLocal("munkalapok",ml.munkalapok); }
      if (uk?.ugyfelek)   { next.ugyfelek=uk.ugyfelek;     updated=true; saveLocal("ugyfelek",uk.ugyfelek); }
      if (updated) setData(next);
      setDrive("ok"); setTimeout(()=>setDrive("idle"),2500);
    })();
  ...
  function handleDeleteConfirm() {
    ...
    const next = removeItem("munkalapok", deleteConfirm.id);
    setData(p=>({...p,munkalapok:next}));
    driveSave("munkalapok",{munkalapok:next});
    setSel(null); setDeleteConfirm(null);
  }
  function handleUjMunkalapSave(ml) {
    const next = addItem("munkalapok", ml);
    setData(p=>({...p,munkalapok:next}));
    setDrive("saving");
    driveSave("munkalapok",{munkalapok:next}).then(ok=>{setDrive(ok?"ok":"error");setTimeout(()=>setDrive("idle"),2500);});
    setUjMunkalapPage(false); setPage("munkalapok");
    if (isMobile) setShowSidebar(false);
  }
  function logout() { setUser(null);setSel(null);setPage("dashboard");setShowSidebar(true);setUjMunkalapPage(false); }
      ...
      {isMobile ? (
          ...
          {showSidebar && !isTelepito ? (
            ...
            <>
              ...
              <MunkalapDetail m={sel} data={data} userRole={user.role}
                onBack={(refresh)=>{ setSel(null); if(refresh){const f=loadLocal("munkalapok");if(f)setData(p=>({...p,munkalapok:f}));} }}
            ...
            <div>
              <div style={{ background:"#2C4A6E",padding:"44px 16px 16px",display:"flex",alignItems:"center",justifyContent:"space-between" }}>
                ...
                <button onClick={logout} style={{ border:"none",background:"rgba(255,255,255,0.1)",color:"#94A3B8",cursor:"pointer",borderRadius:8,padding:"6px 12px",fontSize:12,fontFamily:FONT }}>Kilépés</button>
              </div>
            ...
            <>
              ...
              <PageContent page={page} sel={sel} setSel={setSel} data={data} setData={setData} setPage={setPage} user={user} onNewMunkalap={()=>setUjMunkalapPage(true)} onDelete={handleDeleteRequest}/>
            </>
          )}
        </div>
      ) : (
        <div style={{ display:"flex", minHeight:"100vh", background:C.bg }}>
          ...
          <div style={{ flex:1, overflow:"auto" }}>
            <TopBar title={isMunkalapDetail?sel.id:PAGE_TITLES[page]} user={user} driveStatus={drive} onBack={isMunkalapDetail?()=>setSel(null):undefined} backLabel="Munkalapok"/>
            <PageContent page={page} sel={sel} setSel={setSel} data={data} setData={setData} setPage={setPage} user={user} onNewMunkalap={()=>setUjMunkalapPage(true)} onDelete={handleDeleteRequest}/>
          </div>
        </div>
      )}
