...
import { addKarterites, updateKarterites, deleteKarterites } from "../lib/karterites";
      ...
      <div style={{ background:"#fff", borderRadius:14, border:"1px solid #E2E8F0", marginTop:20, overflow:"hidden" }}>
        ...
        {karteritesek.length === 0 ? (
          ...
          <div style={{ padding:"12px 20px", display:"flex", flexDirection:"column", gap:8 }}>
            {[...karteritesek].sort((a,b)=>new Date(b.createdAt||0)-new Date(a.createdAt||0)).slice(0,10).map(k => (
              <div key={k.id} style={{
                ...
                {isAdmin && (
                  <div style={{ display:"flex", gap:4, flexShrink:0 }}>
                    {k.elfogadott!==true  && <button onClick={()=>{updateKarterites(k.id,{elfogadott:true}); window.dispatchEvent(new CustomEvent("crm-db-updated",{detail:{collection:"karteritesek"}}));}} style={{padding:"3px 8px",background:"#ECFDF5",color:"#059669",border:"none",borderRadius:6,cursor:"pointer",fontSize:11,fontFamily:FONT}}>Elfogad</button>}
                    {k.elfogadott!==false && <button onClick={()=>{updateKarterites(k.id,{elfogadott:false}); window.dispatchEvent(new CustomEvent("crm-db-updated",{detail:{collection:"karteritesek"}}));}} style={{padding:"3px 8px",background:"#FEF2F2",color:"#DC2626",border:"none",borderRadius:6,cursor:"pointer",fontSize:11,fontFamily:FONT}}>Elutasít</button>}
                  </div>
                )}
              </div>
            ))}
          ...
          <div style={{ background:"#fff", borderRadius:16, width:"100%", maxWidth:440, padding:"24px", fontFamily:FONT }}>
            ...
            <div style={{ display:"flex", gap:10, marginTop:18 }}>
              ...
              <button onClick={()=>{
                ...
                addKarterites({ munkalapId:get("munkalapId"), osszeg:Number(get("osszeg")), ok:get("ok"), datum:get("datum"), megjegyzes:get("megjegyzes"), rogzitoSzemely:user?.name||"Admin" });
                window.dispatchEvent(new CustomEvent("crm-db-updated",{detail:{collection:"karteritesek"}}));
                setUjKart(false);
              }} style={{ flex:2, padding:"10px", background:"#DC2626", color:"#fff", border:"none", borderRadius:9, cursor:"pointer", fontWeight:700, fontFamily:FONT }}>Hozzáadás</button>
            </div>
          </div>
        </div>
      )}
...
export default function Dashboard({ data, user }) {
  ...
  // Reaktív frissítés kártérítés változásakor
  useEffect(() => {
