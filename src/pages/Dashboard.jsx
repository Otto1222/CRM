...
export default function Dashboard({ user }) {
  ...
  return (
    <div style={{ padding:"24px 28px", fontFamily:FONT, background: C.bg, minHeight:"100vh" }}>
              ...
              {tableData.map((m, i) => (
                <tr key={m.id} style={{ borderBottom:`1px solid ${C.bg}`, background: i%2===0 ? "#fff" : C.bg,
                  ...
                  <td style={{ padding:"12px 14px" }}>
                    <p style={{ fontWeight:700, color: C.accent, margin:0, fontSize:13 }}>{formatMunkalapAzonosito(m)}</p>
          ...
          <div style={{ padding:"14px 20px", borderBottom:`1px solid ${C.border}`, background: C.bg }}>
            <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", flexWrap:"wrap", gap:14 }}>
              ...
              <div style={{ display:"flex", gap:10, flexWrap:"wrap" }}>
                <div style={{ background: C.successLight, borderRadius:9, padding:"8px 14px", border:`1px solid ${C.success}` }}>
                ...
                <div style={{ background: C.dangerLight, borderRadius:9, padding:"8px 14px", border:`1px solid ${C.danger}` }}>
                ...
                <div style={{ background: projektBilling.totalHaszon >= 0 ? C.successLight : C.dangerLight, borderRadius:9, padding:"8px 14px", border:`1px solid ${projektBilling.totalHaszon >= 0 ? C.success : C.danger}` }}>
              ...
              <tbody>
                {projektBilling.rows.map((row, i) => (
                  <tr key={row.id} style={{ borderBottom:`1px solid ${C.bg}`, background: i%2===0 ? "#fff" : C.bg, borderLeft:`3px solid ${row.nyereseg ? C.success : C.danger}` }}>
                    ...
                    <td style={{ padding:"10px 14px", color: C.text, fontSize:12 }}>{row.nev}</td>
                    <td style={{ padding:"10px 14px", color: C.text }}>{row.fovNev}</td>
      ...
      {isAdmin && !projektBilling && (
        <div style={{ background: C.warningLight, border:`1.5px solid ${C.warning}`, borderRadius:12, padding:"14px 20px", marginTop:20, display:"flex", gap:12, alignItems:"flex-start" }}>
