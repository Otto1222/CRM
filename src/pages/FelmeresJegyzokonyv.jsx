...
<style>
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  .check-box {
    width: 6mm;
    height: 6mm;
    border: 1.5pt solid ${nyilatkozat ? C.success : "#1E3A5F"};
    border-radius: 1.5pt;
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {/* Fejléc */}
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"16px 20px", borderBottom:`1px solid ${C.border}`, background:C.bg }}>
          <div>
        ...
        <div style={{ padding:"20px" }}>
          ...
          <div style={{ display:"flex", gap:12, marginBottom:16 }}>
            {[["📅 Dátum", datumHu], ["👷 Csapat", csapatNev]].map(([l,v]) => (
              <div key={l} style={{ flex:1, background:C.bg, borderRadius:10, padding:"10px 12px", border:`1px solid ${C.border}` }}>
          ...
          <div style={{ border:`2px solid ${nyilatkozat?C.success:C.border}`, borderRadius:12, padding:"14px", marginBottom:14 }}>
            ...
            <p style={{ fontSize:11, color:C.textSub, lineHeight:1.6 }}>{NYILATKOZAT}</p>
          </div>

          {/* Aláírás */}
          <div style={{ border:`1px solid ${C.border}`, borderRadius:12, padding:"12px", marginBottom:16 }}>
            <p style={{ fontSize:12, fontWeight:700, color:C.text, marginBottom:8 }}>✍️ Ügyfél aláírása</p>
            {alairas
              ? <img src={alairas} alt="Aláírás" style={{ border:`1px solid ${C.border}`, borderRadius:8, maxHeight:80, maxWidth:"100%" }}/>
        ...
        {/* Lábléc */}
        <div style={{ padding:"12px 20px", borderTop:`1px solid ${C.border}`, background:C.bg, display:"flex", gap:10, justifyContent:"flex-end" }}>
          <button onClick={onClose} style={{ padding:"9px 16px", borderRadius:9, border:`1.5px solid ${C.border}`, background:"#fff", color:C.textSub, fontWeight:600, fontSize:13, cursor:"pointer", fontFamily:FONT }}>
            Bezárás
          </button>
