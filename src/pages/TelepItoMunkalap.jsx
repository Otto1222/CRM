...
function requiresSerial(nev) {
  if (!nev) return false;
  const lower = nev.toLowerCase().replace(/[áéíóöőúüű]/g,c=>"aeiooouuu"["áéíóöőúüű".indexOf(c)]||c);
  return SERIAL_KEYWORDS.some(k=>{
    const kl=k.toLowerCase().replace(/[áéíóöőúüű]/g,c=>"aeiooouuu"["áéíóöőúüű".indexOf(c)]||c);
...
const FOTO_KAT = [
  ...
  { id:"napelemek",       nev:"Napelemek",                          leiras:"Min. 2 fotó: 2 irányból." },
  { id:"panel_sn",        nev:"Napelem SN számok",                   leiras:"Összes napelem szériaszáma olvashatóan." },
  { id:"optimalizalo",    nev:"Optimalizáló",                        leiras:"min. 1 fotó" },
...
// ─── Segédfüggvény: 0 → törlés ────────────────────────────────
// Ha a felhasználó 0-t ír, töröljük a mezőt (üres lesz)
function normalizeVbfValue(val) {
  if (val === "0" || val === 0) return "";
  return val;
}

// ─── VBF szám input – csak számot fogad, 0 törli ──────────────
function VbfInput({ value, onCommit, unit, piros }) {
  const [local, setLocal] = useState(value ?? "");
  useEffect(() => { setLocal(value ?? ""); }, [value]);
  const empty = local === "" || local === "0";

  function handleChange(e) {
    // Csak számokat és tizedespontot enged
    const v = e.target.value.replace(/[^0-9.,]/g, "");
    setLocal(v);
  }
  function handleBlur() {
    const normalized = normalizeVbfValue(local);
    onCommit(normalized);
  }
...
// ─── Panel típusa: SZÖVEG input (egyedüliként szöveget fogad) ──
function PanelTipusInput({ value, onCommit, piros }) {
  const [local, setLocal] = useState(value ?? "");
  useEffect(() => { setLocal(value ?? ""); }, [value]);
  return (
    <input
  ...
  return (
    <div style={{ background:"#fff", border:`1px solid ${C.border}`, borderRadius:12, padding:"14px 16px", marginBottom:12 }}>
      ...
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
        <span style={{ fontSize:13, color:photos.length>0?C.success:C.muted, fontWeight:600 }}>Feltöltve: {photos.length} db</span>
          ...
          {photos.map((p,i)=>(
            <div key={i} style={{ position:"relative" }}>
              {p.url&&p.type?.startsWith("image")
                ? <img src={p.url} style={{ width:64,height:64,objectFit:"cover",borderRadius:8,border:`1px solid ${C.border}` }}/>
...
function FelhasznaltAnyagokTab({ munkalapId, meglevoAnyagok, onSave }) {
  ...
  function add() {
    ...
    setAnyagok(p=>[...p, { id:`a_${Date.now()}`, nev:ujNev.trim(), menny:ujMenny, egyseg:ujEgyseg, sorozatszam:requiresSerial(ujNev)?"":null, kotelezőSerial:requiresSerial(ujNev) }]);
    setUjNev(""); setUjMenny(1);
  }

  function save() {
    const hiany = anyagok.filter(a=>a.kotelezőSerial&&(!a.sorozatszam||a.sorozatszam.trim()===""));
    if (hiany.length>0) { setHiba(`Hiányzó sorozatszám: ${hiany.map(a=>a.nev).join(", ")}`); return; }
    setHiba("");
  ...
  return (
    <div style={{ padding:"16px", background:"#F1F5F9" }}>
      <p style={{ fontSize:13, color:C.muted, marginBottom:16, lineHeight:1.6 }}>
        Felhasznált anyagok listája. Kék keretes tételeknél kötelező a sorozatszám.
      </p>
      ...
      {anyagok.map(a=>(
        <div key={a.id} style={{ background:"#fff", border:`1.5px solid ${a.kotelezőSerial?"#2563EB30":C.border}`, borderRadius:12, padding:"12px 14px", marginBottom:10 }}>
          <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:a.kotelezőSerial?10:0 }}>
            ...
            <button onClick={()=>setAnyagok(p=>p.filter(x=>x.id!==a.id))} style={{ border:"none",background:"none",cursor:"pointer",color:C.danger,flexShrink:0 }}><Trash2 size={14}/></button>
          </div>
          {a.kotelezőSerial && (
            <div>
              <label style={{ fontSize:11, color:C.accent, fontWeight:700, textTransform:"uppercase", letterSpacing:.8 }}>Sorozatszám (kötelező)</label>
              <input value={a.sorozatszam||""} onChange={e=>setAnyagok(p=>p.map(x=>x.id===a.id?{...x,sorozatszam:e.target.value}:x))}
      ...
      <div style={{ border:`1.5px dashed ${C.border}`,borderRadius:12,padding:14,background:"#fff",marginBottom:16 }}>
        ...
        <input value={ujNev} onChange={e=>setUjNev(e.target.value)} onKeyDown={e=>e.key==="Enter"&&add()} placeholder="Anyag / eszköz neve…"
...
export default function TelepItoMunkalap({ m, data, onBack }) {
  ...
  function handleBefejezesKezdete() {
    ...
    if (osszesFoto === 0) {
      alert("⚠️ Lezárás sikertelen!\n\nNincs feltöltve egyetlen fotó sem.\nTölts fel legalább 1 fotót a Fotók fülön.");
      return;
    }

    // 3. Megjegyzés kötelező (min. 10 karakter)
    if (!megjegyzes || megjegyzes.trim().length < 10) {
      alert("⚠️ Lezárás sikertelen!\n\nA megjegyzés mező kötelező és legalább 10 karakter legyen.\nÍrd le röviden a munkavégzés összefoglalóját.");
      return;
    }

    // 4. Hiányos fotó kategóriák indoklása
    ...
    if (!mindOk) {
      ...
      alert("⚠️ Lezárás sikertelen!\n\nA következő kategóriáknál nincs ok megadva:\n" + missing.map(k=>"• "+k.nev).join("\n") + "\n\nAz Ellenőrzés fülön add meg az okot minden fotó nélküli kategóriához!");
      return;
    }
  ...
  async function handleBefejezes(alairasData) {
    ...
    if (alairasData) {
      updateItem("munkalapok", m.id, {
        alairas: { dataUrl:alairasData.alairasDataUrl, datum:alairasData.alairasDatum, szoveg:alairasData.szoveg }
      });
    }
  ...
  async function uploadFotokToDrive() {
    ...
    try {
      ...
      const osszesFoto = Object.entries(fotok).flatMap(([katId,photos])=>photos.filter(p=>p.fileObj||p.file||(p.url&&p.url.startsWith("blob:"))).map(p=>({...p,katId})));
      let feltoltve = 0;
      for (const foto of osszesFoto) {
        try {
          ...
          if (fileSource) {
            base64 = await new Promise(res=>{ const r=new FileReader(); r.onload=e=>res(e.target.result.split(",")[1]); r.onerror=()=>res(""); r.readAsDataURL(fileSource); });
          } else if (foto.url?.startsWith("blob:")) {
            try { const resp=await fetch(foto.url); const blob=await resp.blob(); base64=await new Promise(res=>{const r=new FileReader();r.onload=e=>res(e.target.result.split(",")[1]);r.onerror=()=>res("");r.readAsDataURL(blob);}); } catch{}
          }
          if (!base64) continue;
          await fetch(scriptUrl, { method:"POST", mode:"no-cors", headers:{"Content-Type":"application/json"}, body:JSON.stringify({ action:"saveFoto", munkalapId:m.id, fotoNev:foto.name, fotoBase64:base64, mimeType:foto.type||"image/jpeg" }) });
          feltoltve++;
          setProgressMsg(`Fotók feltöltése… (${feltoltve}/${osszesFoto.length})`);
        } catch{}
      }
  ...
  const InfoTab = () => {
    ...
    return (
      <div style={{ padding:"0 16px 16px",background:"#F1F5F9" }}>
        ...
        {/* Megjegyzés mező – kötelező lezáráshoz */}
        <div style={{ marginTop:16 }}>
          <p style={{ fontSize:12,color:"#64748B",marginBottom:4,fontWeight:600 }}>
            Megjegyzés / munkavégzés összefoglalója <span style={{ color:"#DC2626" }}>*kötelező</span>
          </p>
          <textarea
            value={megjegyzes}
            onChange={e=>setMegjegyzes(e.target.value)}
            onBlur={()=>{ if(megjegyzes.trim()) { updateItem("munkalapok",m.id,{megjegyzes:megjegyzes.trim()}); window.dispatchEvent(new CustomEvent("crm-db-updated",{detail:{collection:"munkalapok"}})); } }}
            placeholder="Írd le röviden a munkavégzés menetét, észrevételeket… (min. 10 karakter)"
            rows={4}
            style={{ width:"100%",padding:"10px 12px",border:`1.5px solid ${!megjegyzes||megjegyzes.trim().length<10?"#EF4444":C.border}`,borderRadius:9,fontSize:14,fontFamily:FONT,color:C.text,outline:"none",background:"#fff",resize:"vertical" }}
          />
          {megjegyzes.trim().length > 0 && megjegyzes.trim().length < 10 &&
            <p style={{ fontSize:11,color:"#DC2626",marginTop:4 }}>Még {10-megjegyzes.trim().length} karakter szükséges</p>}
        </div>
  ...
  // ─── VBF TAB – 0 értéknél mező törlése, csak szám, panelTipus szöveg ───
  const VbfTab = () => (
    <div style={{ padding:"16px",background:"#F1F5F9" }}>
      {figy&&<div style={{ background:"#FEF2F2",border:`1px solid #FECACA`,borderRadius:10,padding:"10px 14px",marginBottom:14,fontSize:13,color:C.danger,display:"flex",alignItems:"center",gap:8 }}>
        <AlertTriangle size={16}/>Ha valamelyik mező nem releváns, hagyd üresen – az üres mező nem kötelező a mentéshez.
      </div>}
      <div style={{ background:"#EFF6FF",border:`1px solid #BFDBFE`,borderRadius:10,padding:"10px 14px",marginBottom:14,fontSize:12,color:"#1D4ED8" }}>
        💡 Ha egy érték 0, a mezőt üresen hagyd – a rendszer automatikusan kihagyja.
      </div>
      ...
      <MeroSzakasz title="">
        <EgyMero label="Smart meter" value={vbf.smartMeter} onCommit={v=>updVbf("smartMeter",null,v)} unit="DB" piros={figy}/>
        <EgyMero label="AKKU" value={vbf.akku} onCommit={v=>updVbf("akku",null,v)} unit="DB" piros={figy}/>
      </MeroSzakasz>
      ...
      <MeroSzakasz title="Inverter pontos adatok">
        <EgyMero label="Névleges teljesítménye" value={vbf.inverterNevleges} onCommit={v=>updVbf("inverterNevleges",null,v)} unit="kVA" piros={figy}/>
      </MeroSzakasz>
  ...
  const FOTO_HIANY_OKOK_LIST = ["Nincs ilyen eszköz","Nem látható","Nem releváns a munkatípushoz"];

  const EllenorzesTab = () => {
    const vbfOk = !checkHianyos();
    const osszesFoto = Object.values(fotok).reduce((s,a)=>s+(a.length||0),0);
    const megjegyzesFoto = megjegyzes.trim().length >= 10;
    const hianyosKat = FOTO_KAT.filter(k=>(fotok[k.id]||[]).length===0);
    ...
    return (
      <div style={{ padding:"16px",background:"#F1F5F9",paddingBottom:80 }}>
        <div style={{ background:"#fff",border:`1px solid ${C.border}`,borderRadius:12,padding:16,marginBottom:16 }}>
          ...
          {[
            { label:"VBF Jegyzőkönyv kitöltve",          ok:vbfOk,          info:"Töltsd ki a VBF fülön" },
            { label:`Fotók feltöltve (${osszesFoto} db)`, ok:osszesFoto>0,   info:"Min. 1 fotó szükséges" },
            { label:"Megjegyzés megadva",                 ok:megjegyzesFoto, info:"Min. 10 karakter az Infó fülön" },
            { label:`Hiányos kategóriák indoklása`,       ok:mindenKatOk||hianyosKat.length===0, info:"Minden fotó nélküli kategóriához válassz okot" },
          ].map(item=>(
        ...
        {hianyosKat.length > 0 && (
          <div style={{ background:"#fff",border:"1.5px solid #FED7AA",borderRadius:12,padding:16,marginBottom:16 }}>
            <p style={{ fontSize:14,fontWeight:700,color:"#C2410C",marginBottom:12 }}>⚠️ Hiányos fotó kategóriák – válassz okot</p>
            {hianyosKat.map(k=>(
              <div key={k.id} style={{ marginBottom:10 }}>
                <p style={{ fontSize:13,fontWeight:700,color:"#0F172A",marginBottom:5 }}>{k.nev}</p>
        ...
        {!lezarhatoE && (
          <p style={{ fontSize:12,color:"#DC2626",textAlign:"center",marginTop:8,fontWeight:600 }}>
            Hiányzó feltételek piros háromszöggel jelölve fent.
          </p>
        )}
      </div>
    );
  ...
  return (
    <div style={{ minHeight:"100vh",background:"#F1F5F9",fontFamily:FONT }}>
      ...
      {activeTab===3&&<FelmeresFotok munkalapId={m.id} status={m.status} userRole="Telepítő"/>}
      ...
      {showAlairas&&(
        <AlairasModal m={m} userRole="Telepítő" onClose={()=>setShowAlairas(false)} onSave={handleBefejezes}/>
      )}
    </div>
  );
}
