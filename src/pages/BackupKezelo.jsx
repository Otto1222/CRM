...
export default function BackupKezelo({ userRole }) {
  ...
  return (
    <div style={{padding:"24px 28px",fontFamily:FONT,maxWidth:860}}>
      ...
      {uzenet && (
        <div style={{marginBottom:16,padding:"10px 14px",borderRadius:10,background:uzenet.ok?C.successLight:C.dangerLight,border:`1.5px solid ${uzenet.ok?C.success:C.dangerLight}`,fontSize:13,fontWeight:700,color:uzenet.ok?C.success:C.danger}}>
          {uzenet.szoveg}
        </div>
      )}

      <div style={{background:C.warningLight,border:`1.5px solid ${C.warningLight}`,borderRadius:12,padding:"14px 16px",marginBottom:20,display:"flex",gap:10}}>
