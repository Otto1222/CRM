            ...
            {filtered.length === 0
              ...
              : filtered.map(o => (
                  <div
                    ...
                    style={{ padding:"10px 14px", cursor:"pointer", background: (o.id || o.value) === value ? C.accentLight : "transparent", borderBottom:`1px solid ${C.bg}`, transition:"background .1s" }}
    ...
    <div style={{ padding:"24px 28px", fontFamily:FONT, maxWidth:960 }}>
      ...
      {szurtLista.length === 0 ? (
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
          {szurtLista.map(k => {
            const st = getKarteritesStatus(k);
            const borderColor = st === "elfogadott" ? C.success : st === "elutasitott" ? C.dangerLight : C.border;
            return (
                ...
                <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", gap:12 }}>
                  ...
                  {isAdmin && (
                    <div style={{ display:"flex", flexDirection:"column", gap:6, flexShrink:0 }}>
                      {st !== "elfogadott" && (
                        <button onClick={() => handleDontse(k.id, "elfogadott")} style={{ display:"flex", alignItems:"center", gap:5, padding:"6px 12px", background:C.successLight, color:C.success, border:`1.5px solid ${C.success}`, borderRadius:8, cursor:"pointer", fontSize:12, fontWeight:700, fontFamily:FONT }}>
                          <CheckCircle2 size={13} /> Elfogad
                        </button>
                      )}
                      {st !== "elutasitott" && (
                        <button onClick={() => handleDontse(k.id, "elutasitott")} style={{ display:"flex", alignItems:"center", gap:5, padding:"6px 12px", background:C.dangerLight, color:C.danger, border:`1.5px solid ${C.dangerLight}`, borderRadius:8, cursor:"pointer", fontSize:12, fontWeight:700, fontFamily:FONT }}>
                          <XCircle size={13} /> Elutasít
                        </button>
                      )}
                      {st === "elfogadott" && (
                        <button onClick={() => handleDontse(k.id, "fuggoben")} style={{ display:"flex", alignItems:"center", gap:5, padding:"6px 12px", background:C.warningLight, color:C.warning, border:`1.5px solid ${C.warningLight}`, borderRadius:8, cursor:"pointer", fontSize:12, fontWeight:700, fontFamily:FONT }}>
                          ↩ Visszavon
                        </button>
                      )}
