...
const SYNC_COLORS = {
  ...
  error:   { bg: C.dangerHover,               color: "#fff" },
};
                  ...
                  <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
                    ...
                    {ev.type === "manual" && (
                      <button onClick={() => handleDeleteEvent(ev.id)}
                        style={{ fontSize: 11, padding: "3px 10px", background: `${C.danger}22`, color: C.danger, border: `1px solid ${C.danger}40`, borderRadius: 6, cursor: "pointer" }}>
                        Töröl
                      </button>
                    )}
                  </div>
...
const INPUT_STYLE = {
  ...
  border: `1px solid ${C?.border || C.text}`,
  background: C?.card || C.text, color: C?.text || C.bg,
