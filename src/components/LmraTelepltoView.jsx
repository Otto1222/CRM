 ...
 *   2. Résztvevők hozzáadása (legördülő + kézi + "Mentés csapatba")
 *   3. Mindenki aláír saját aláírás-mezőben
 *   4. LMRA lezárása
 */
...
import {
  ...
  Users, UserPlus, Lock, AlertTriangle, ChevronDown,
} from "lucide-react";
import { C, FONT, FONT_HEADING } from "../lib/constants";
import {
  loadLmraRec, addResztvevo, removeResztvevo, saveSignature, closeLmra,
...
function AlairasMezo({ nev, onSave, onSkip }) {
...
export default function LmraTelepltoView({ munkalap, currentUser, onClose, onComplete }) {
  ...
  const [step, setStep] = useState("kockazatok"); // kockazatok | resztvevok | alairas | kesz
  const [ujNev, setUjNev] = useState("");
  const [ujNevFromList, setUjNevFromList] = useState("");
  const [mentesToggle, setMentesToggle] = useState({});
  const [kivalasztottIdx, setKivalasztottIdx] = useState(0);
  const availableTagok = getRelevantTagok(munkalap);
  const csapatok       = getAktivTeleppCsapatok();
    ...
    return (
      <div style={{ position: "fixed", inset: 0, zIndex: 3000, background: "rgba(0,0,0,0.82)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 20 }}>
        <div style={{ background: "#fff", borderRadius: 16, padding: "32px 28px", maxWidth: 420, textAlign: "center" }}>
          ...
          <p style={{ fontWeight: 800, fontSize: 17, color: C.text, margin: "0 0 8px" }}>
            LMRA nincs előkészítve
          </p>
          <p style={{ fontSize: 13, color: C.muted, margin: "0 0 20px", lineHeight: 1.6 }}>
            ...
            Kérj meg egy PM-et, hogy töltse ki a kockázatokat, majd zárja le.
          </p>
          <button
            onClick={onClose}
            style={{ padding: "11px 28px", background: C.accent, color: "#fff", border: "none", borderRadius: 10, cursor: "pointer", fontWeight: 700, fontSize: 14, fontFamily: FONT }}
          >
            Bezárás
          </button>
        </div>
  ...
  const alairtDb     = resztvevok.filter(r => r.signed).length;
  const mindenAlairt = resztvevok.length > 0 && resztvevok.every(r => r.signed);
  ...
  function handleSaveToTeam(resztvevoId) {
    const csList = csapatok;
    if (csList.length === 0) {
      alert("Nincs Telepítő csapat létrehozva. Hozz létre egyet a Beállítások → Telepítő csapatok menüpontban.");
      return;
    }
    let csapatId;
    if (csList.length === 1) {
      csapatId = csList[0].id;
    } else {
      const nevek = csList.map(c => c.nev).join("\n");
      const valasz = window.prompt(`Melyik csapathoz mentse? (Írd be a nevet pontosan):\n\n${nevek}`);
      csapatId = csList.find(c => c.nev === valasz?.trim())?.id;
      if (!csapatId) { alert("Nem található ilyen csapat."); return; }
    }
    const tag = saveNewTagToTeam(munkalap.id, resztvevoId, csapatId);
    if (tag) setRec(loadLmraRec(munkalap.id));
  }
  ...
  function handleSign(resztvevoId, dataUrl) {
    ...
    if (updated) {
      setRec(updated);
      // Következő aláíratlan személyre ugrás
      const remaining = (updated.resztvevok || []).findIndex(r => !r.signed);
      if (remaining === -1) {
        setKivalasztottIdx(0);
      } else {
        setKivalasztottIdx(remaining);
      }
    }
  }
  ...
  const akotuSzemely = resztvevok[kivalasztottIdx] || resztvevok.find(r => !r.signed);
      ...
      <div style={{ position: "fixed", inset: 0, zIndex: 3000, background: "rgba(0,0,0,0.82)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 20 }}>
        <div style={{ background: "#fff", borderRadius: 16, padding: "28px 24px", maxWidth: 440, width: "100%", textAlign: "center" }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 7, justifyContent: "center", marginBottom: 20 }}>
            {resztvevok.map(r => (
              <span key={r.id} style={{ fontSize: 13, background: "#ECFDF5", color: "#059669", padding: "4px 13px", borderRadius: 20, fontWeight: 700 }}>
                ✓ {r.nev}
              </span>
            ))}
          </div>
    ...
    <div style={{ position: "fixed", inset: 0, zIndex: 3000, background: "rgba(0,0,0,0.82)", display: "flex", flexDirection: "column" }}>
      <div style={{ background: "#fff", flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
            ...
            {STEPS.map((id, i) => {
              ...
              return (
                <div
                  ...
                  style={{
                    flex: 1, padding: "9px 4px", display: "flex", alignItems: "center", justifyContent: "center",
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "14px 16px 20px" }}>
          ...
          {/* ── LÉPÉS 1: KOCKÁZATOK (csak olvasható) ── */}
          {step === "kockazatok" && (
            <div>
              ...
              {kivalasztott.length === 0 ? (
                <div style={{ textAlign: "center", padding: "24px 0", color: C.muted, fontSize: 13 }}>
                  Nincsenek kockázatok kijelölve.
                </div>
              ) : (
                <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
                  {kivalasztott.map(k => (
                    <div
                      key={k.id}
                      style={{ background: "#FEF2F2", border: "1.5px solid #FECACA", borderRadius: 10, padding: "10px 14px" }}
                    >
              ...
              {/* Nem kiválasztott kockázatok – szürkén */}
              {(rec.kockazatok || []).some(k => !k.kivalasztva) && (
                <details style={{ marginTop: 14 }}>
                  <summary style={{ fontSize: 12, color: C.muted, cursor: "pointer", marginBottom: 7 }}>
                    Nem vonatkozó kockázatok mutatása ({(rec.kockazatok || []).filter(k => !k.kivalasztva).length} db)
                  </summary>
          ...
          {step === "resztvevok" && (
            <div>
              <p style={{ fontSize: 13, color: C.muted, marginBottom: 14 }}>
                Add hozzá a ma dolgozó csapattagokat. Mindenki saját neve alatt fog aláírni.
              </p>
                ...
                <div style={{ marginBottom: 16 }}>
                  {resztvevok.map(r => (
                    <div key={r.id} style={{
                      ...
                      padding: "9px 12px", background: r.signed ? "#ECFDF5" : "#F8FAFC",
                      border: `1px solid ${r.signed ? "#86EFAC" : C.border}`,
                      borderRadius: 9, marginBottom: 6,
                    }}>
                      <span style={{ flex: 1, fontSize: 14, fontWeight: 600, color: r.signed ? "#059669" : C.text }}>
                        {r.signed ? "✓ " : ""}{r.nev}
                        {r.signed && r.signedAt && (
                          <span style={{ fontWeight: 400, fontSize: 11, marginLeft: 8, color: C.muted }}>
                      ...
                      {!r.signed && (
                        <>
                          {r.addedManually && !r.savedToTeam && csapatok.length > 0 && (
                            <button
                              onClick={() => handleSaveToTeam(r.id)}
                              style={{ fontSize: 11, color: "#7C3AED", background: "#F5F3FF", border: "1px solid #DDD6FE", borderRadius: 6, padding: "3px 8px", cursor: "pointer", fontFamily: FONT, fontWeight: 600 }}
                            >
                              + Mentés csapatba
                            </button>
                          )}
                          <button
                            ...
                            style={{ padding: "4px 7px", background: "#FEF2F2", border: "none", borderRadius: 6, cursor: "pointer", color: "#DC2626" }}
                          >
                            <X size={13} />
                          </button>
                        </>
              ...
              {availableTagok.length > 0 && (
                <div style={{ marginBottom: 14 }}>
                  <p style={{ fontSize: 12, fontWeight: 700, color: C.muted, margin: "0 0 7px", textTransform: "uppercase", letterSpacing: .5 }}>
                    Csapatból választás
                  </p>
                  <div style={{ display: "flex", gap: 7 }}>
                    <div style={{ flex: 1, position: "relative" }}>
                      <select
                        value={ujNevFromList}
                        onChange={e => setUjNevFromList(e.target.value)}
                      ...
                      >
                        ...
                        {availableTagok
                          .filter(t => !resztvevok.some(r => r.nev === t.nev))
                          .map(t => (
                            <option key={t.id} value={t.nev}>{t.nev}</option>
                          ))}
                      </select>
                    </div>
                    <button
                      onClick={handleAddFromList}
                      disabled={!ujNevFromList}
                      style={{ padding: "10px 18px", border: "none", background: ujNevFromList ? C.accent : "#E2E8F0", color: "#fff", borderRadius: 9, cursor: ujNevFromList ? "pointer" : "default", fontWeight: 700, fontSize: 15 }}
                    >
                      +
                    </button>
                  </div>
              ...
              <div>
                <p style={{ fontSize: 12, fontWeight: 700, color: C.muted, margin: "0 0 7px", textTransform: "uppercase", letterSpacing: .5 }}>
                  Kézi beírás (ha nincs a listában)
                </p>
                <div style={{ display: "flex", gap: 7 }}>
                  <input
                    value={ujNev}
                  ...
                  <button
                    ...
                    disabled={!ujNev.trim()}
                    style={{ padding: "10px 18px", border: "none", background: ujNev.trim() ? "#059669" : "#E2E8F0", color: "#fff", borderRadius: 9, cursor: ujNev.trim() ? "pointer" : "default", fontWeight: 700, fontSize: 15 }}
                  >
                    +
                  </button>
                </div>
            ...
            <div>
              ...
              <div style={{ background: "#F0FDF4", border: "1px solid #86EFAC", borderRadius: 10, padding: "10px 14px", marginBottom: 14, fontSize: 12 }}>
                <p style={{ fontWeight: 700, color: "#166534", margin: "0 0 4px" }}>
                  ✅ {kivalasztott.length} kockázat ellenőrizve
                </p>
                <p style={{ color: "#166534", margin: 0 }}>
                  Aláírásommal igazolom, hogy a kockázatokat és a munkavégzés feltételeit megismertem,
              ...
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 14 }}>
                {resztvevok.map((r, i) => (
                  <span
                    key={r.id}
                    onClick={() => !r.signed && setKivalasztottIdx(i)}
                    style={{
                      fontSize: 12, fontWeight: 700, padding: "5px 13px", borderRadius: 20, cursor: r.signed ? "default" : "pointer",
              ...
              {/* Aktuális aláírás canvas */}
              {akotuSzemely && !akotuSzemely.signed && (
                <AlairasMezo
                  nev={akotuSzemely.nev}
                  onSave={dataUrl => handleSign(akotuSzemely.id, dataUrl)}
                />
              )}

              {mindenAlairt && (
                <div style={{ background: "#ECFDF5", border: "1px solid #86EFAC", borderRadius: 10, padding: "12px 16px", marginTop: 14, textAlign: "center" }}>
                  <p style={{ fontWeight: 800, fontSize: 15, color: "#059669", margin: 0 }}>
                    ✅ Mindenki aláírt! Az LMRA lezárható.
                  </p>
                </div>
              )}

              <p style={{ fontSize: 11, color: C.muted, textAlign: "center", marginTop: 10 }}>
                {alairtDb} / {resztvevok.length} aláírás
              </p>
            </div>
          )}
            ...
            <div style={{ textAlign: "center", padding: "28px 0" }}>
              ...
              <div style={{ display: "flex", gap: 7, justifyContent: "center", flexWrap: "wrap", marginBottom: 16 }}>
                {resztvevok.filter(r => r.signed).map(r => (
                  <span key={r.id} style={{ fontSize: 13, background: "#ECFDF5", color: "#059669", padding: "4px 14px", borderRadius: 20, fontWeight: 700 }}>
                    ✓ {r.nev}
                  </span>
                ))}
              </div>
        ...
        <div style={{ padding: "12px 16px", borderTop: `1px solid ${C.border}`, flexShrink: 0, background: "#fff" }}>
          ...
          {step === "resztvevok" && (
            <div style={{ display: "flex", gap: 9 }}>
              ...
              <button
                onClick={() => setStep("alairas")}
                disabled={resztvevok.length === 0}
                style={{ flex: 2, padding: "13px", border: "none", borderRadius: 10, background: resztvevok.length > 0 ? C.accent : "#E2E8F0", color: "#fff", fontWeight: 800, fontSize: 15, cursor: resztvevok.length > 0 ? "pointer" : "default", display: "flex", alignItems: "center", justifyContent: "center", gap: 7, fontFamily: FONT }}
              >
          ...
          {step === "alairas" && (
            <div style={{ display: "flex", gap: 9 }}>
              ...
              <button
                onClick={handleLezaras}
                disabled={!mindenAlairt}
