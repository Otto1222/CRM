...
function TetelSor({ tetel, onChange, onDelete }) {
  ...
  return (
    <div style={{ background:"#F8FAFC", border:"1px solid #E2E8F0", borderRadius:9, padding:"10px 12px", marginBottom:8 }}>
      <div style={{ display:"grid", gridTemplateColumns:"2fr 1.5fr 1.5fr auto", gap:8, alignItems:"center" }}>
        <div>
          <label style={{ fontSize:10, fontWeight:700, color:"#64748B", display:"block", marginBottom:3 }}>Tétel típusa</label>
        ...
        <div>
          <label style={{ fontSize:10, fontWeight:700, color:"#64748B", display:"block", marginBottom:3 }}>Árlogika</label>
        ...
        <div>
          <label style={{ fontSize:10, fontWeight:700, color:"#64748B", display:"block", marginBottom:3 }}>
          ...
          {tetel.arlogikaTipus === "tartomany_fix" ? (
            <p style={{ fontSize:11, color:"#94A3B8", paddingTop:8 }}>Lent szerkeszthető</p>
          ) : tetel.arlogikaTipus === "kezi" ? (
            <p style={{ fontSize:11, color:"#94A3B8", paddingTop:8 }}>Kézi bevitel</p>
          ) : (
        ...
        <button onClick={onDelete} style={{ padding:"6px 8px", background:"#FEF2F2", color:"#DC2626", border:"none", borderRadius:7, cursor:"pointer", marginTop:14 }}>
          <Trash2 size={13}/>
        </button>
      </div>
      ...
      {tetel.arlogikaTipus === "tartomany_fix" && (
        <div style={{ marginTop:10, paddingTop:10, borderTop:"1px solid #E2E8F0" }}>
          <div style={{ display:"flex", justifyContent:"space-between", marginBottom:6 }}>
            <span style={{ fontSize:11, fontWeight:700, color:"#64748B" }}>Darabszám tartományok</span>
            <button onClick={()=>u("tartomanyok",[...(tetel.tartomanyok||[]),{tol:1,ig:999,ar:0}])}
              style={{ fontSize:11, padding:"3px 10px", background:"#EFF6FF", color:"#2563EB", border:"none", borderRadius:6, cursor:"pointer" }}>
              + Tartomány
            </button>
          </div>
          {(tetel.tartomanyok||[]).map((tar,i)=>(
            <div key={i} style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1.5fr auto", gap:6, marginBottom:5, alignItems:"center" }}>
              <div>
                <label style={{ fontSize:9, color:"#94A3B8" }}>Tól (db)</label>
              ...
              <div>
                <label style={{ fontSize:9, color:"#94A3B8" }}>Ig (db)</label>
              ...
              <div>
                <label style={{ fontSize:9, color:"#94A3B8" }}>Ár (Ft)</label>
              ...
              <button onClick={()=>u("tartomanyok",tetel.tartomanyok.filter((_,j)=>j!==i))}
                style={{ padding:"5px 7px", background:"#FEF2F2", color:"#DC2626", border:"none", borderRadius:6, cursor:"pointer", marginTop:14 }}>
                <Trash2 size={11}/>
              </button>
            </div>
          ))}
...
function MtSor({ mt, onSave, onDelete }) {
  ...
  return (
    <div style={{ background:"#fff", border:`1.5px solid ${mt.aktiv?"#E2E8F0":"#F1F5F9"}`, borderRadius:12, marginBottom:10, overflow:"hidden", opacity:mt.aktiv?1:.65 }}>
      <div style={{ display:"flex", alignItems:"center", gap:12, padding:"13px 18px", cursor:"pointer" }} onClick={()=>setOpen(o=>!o)}>
        <div style={{ flex:1 }}>
          <div style={{ display:"flex", gap:10, alignItems:"center" }}>
            ...
            <span style={{ fontSize:11, background:mt.aktiv?"#DCFCE7":"#F1F5F9", color:mt.aktiv?"#166534":"#94A3B8", padding:"2px 9px", borderRadius:20, fontWeight:600 }}>{mt.aktiv?"Aktív":"Inaktív"}</span>
            <span style={{ fontSize:11, color:"#94A3B8" }}>{(mt.beveteliTetelek||[]).length} bevételi tétel</span>
          </div>
          {mt.leiras && <p style={{ fontSize:11, color:"#94A3B8", margin:"2px 0 0" }}>{mt.leiras}</p>}
        </div>
        <div style={{ display:"flex", gap:6 }}>
          <button onClick={e=>{e.stopPropagation();setEdit(true);setOpen(true);}} style={{ padding:"5px 10px", background:"#EFF6FF", color:"#2563EB", border:"none", borderRadius:7, cursor:"pointer", fontSize:11 }}>Szerkeszt</button>
          <button onClick={e=>{e.stopPropagation();onDelete(mt.id);}} style={{ padding:"5px 8px", background:"#FEF2F2", color:"#DC2626", border:"none", borderRadius:7, cursor:"pointer" }}><Trash2 size={13}/></button>
          ...
          {edit ? (
            <>
              <div style={{ display:"grid", gridTemplateColumns:"2fr 1fr", gap:10, marginBottom:14 }}>
                <div>
                  <label style={{ fontSize:10, fontWeight:700, color:"#64748B", display:"block", marginBottom:3 }}>Munkatípus neve *</label>
                  <input value={form.nev} onChange={e=>setForm(p=>({...p,nev:e.target.value}))} style={inp}/>
                </div>
                <div>
                  <label style={{ fontSize:10, fontWeight:700, color:"#64748B", display:"block", marginBottom:3 }}>Aktív</label>
                ...
                <div style={{ gridColumn:"span 2" }}>
                  <label style={{ fontSize:10, fontWeight:700, color:"#64748B", display:"block", marginBottom:3 }}>Leírás</label>
                  <input value={form.leiras} onChange={e=>setForm(p=>({...p,leiras:e.target.value}))} style={inp}/>
                </div>
              </div>

              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10 }}>
                <span style={{ fontSize:12, fontWeight:700, color:"#374151" }}>Bevételi tételek</span>
                <button onClick={addTetel} style={{ display:"flex", alignItems:"center", gap:5, padding:"5px 12px", background:"#2563EB", color:"#fff", border:"none", borderRadius:7, cursor:"pointer", fontSize:11, fontFamily:FONT }}>
              ...
              {form.beveteliTetelek.length===0&&<p style={{ fontSize:12, color:"#94A3B8", fontStyle:"italic", marginBottom:10 }}>Még nincs bevételi tétel – adj hozzá egyet</p>}
              ...
              <div style={{ display:"flex", gap:8, justifyContent:"flex-end", marginTop:12 }}>
                ...
                <button onClick={()=>{onSave(mt.id,form);setEdit(false);}} disabled={!form.nev.trim()} style={{ padding:"7px 16px", background:form.nev.trim()?"#059669":"#CBD5E1", color:"#fff", border:"none", borderRadius:8, cursor:"pointer", fontWeight:700, fontFamily:FONT }}>Mentés</button>
              </div>
            </>
          ) : (
            <div>
              {(mt.beveteliTetelek||[]).length===0
                ? <p style={{ fontSize:12, color:"#94A3B8", fontStyle:"italic" }}>Nincs bevételi tétel definiálva</p>
                : (mt.beveteliTetelek||[]).map((t,i)=>{
                    ...
                    return (
                      <div key={i} style={{ display:"flex", gap:12, padding:"7px 0", borderBottom:"1px solid #F1F5F9" }}>
                        <span style={{ fontSize:12, fontWeight:600, color:"#374151", minWidth:220 }}>{info?.label||t.tetelTipusId}</span>
                        <span style={{ fontSize:12, color:"#64748B" }}>{arl?.label}</span>
                        {t.arlogikaTipus==="darab_egysegar"&&<span style={{ fontSize:12, color:"#2563EB" }}>{ft(t.egysegAr)}/db</span>}
                        {t.arlogikaTipus==="fix_osszeg"&&<span style={{ fontSize:12, color:"#2563EB" }}>{ft(t.fixOsszeg)}</span>}
                        {t.arlogikaTipus==="tartomany_fix"&&<span style={{ fontSize:12, color:"#7C3AED" }}>{(t.tartomanyok||[]).length} tartomány</span>}
                      </div>
                    );
                  })
              }
    ...
    <div style={{ padding:"24px 28px", fontFamily:FONT, maxWidth:900 }}>
      <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", marginBottom:20 }}>
        <div>
          ...
          <p style={{ fontSize:13, color:"#64748B", margin:0 }}>Admin szerkeszthető munkatípusok és bevételi tételdefiníciók</p>
        </div>
        {isAdmin && (
        ...
        <div style={{ background:"#F0F9FF", border:"2px solid #93C5FD", borderRadius:12, padding:"16px", marginTop:12 }}>
          ...
          <div style={{ display:"flex", gap:8 }}>
            ...
            <button onClick={()=>{ if(!ujNev.trim()) return; createMunkatipus({nev:ujNev.trim(),beveteliTetelek:[]}); setMtk(loadMunkatipusok()); setUjOpen(false); setUjNev(""); }} disabled={!ujNev.trim()}
              style={{ padding:"8px 16px", background:ujNev.trim()?"#2563EB":"#CBD5E1", color:"#fff", border:"none", borderRadius:8, cursor:"pointer", fontWeight:700, fontFamily:FONT }}>
              Létrehozás
            </button>
          </div>
        </div>
