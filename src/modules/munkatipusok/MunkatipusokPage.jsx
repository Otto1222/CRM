...
const inp = { width:"100%", boxSizing:"border-box", padding:"8px 11px", border:`1.5px solid ${C.border}`, borderRadius:8, fontSize:13, fontFamily:"inherit", outline:"none" };

// ─── Bevételi tétel szerkesztő egy sorra ─────────────────────
function TetelSor({ tetel, onChange, onDelete }) {
  const u = (k, v) => onChange({ ...tetel, [k]: v });
  return (
    <div style={{ background:C.bg, border:`1px solid ${C.border}`, borderRadius:9, padding:"10px 12px", marginBottom:8 }}>
          ...
          {edit ? (
            <>
              ...
              <div style={{ display:"flex", gap:8, justifyContent:"flex-end", marginTop:12 }}>
                <button onClick={()=>{setEdit(false);setForm({nev:mt.nev,leiras:mt.leiras||"",aktiv:mt.aktiv??true,beveteliTetelek:mt.beveteliTetelek||[]});}} style={{ padding:"7px 14px", border:`1.5px solid ${C.border}`, borderRadius:8, background:"#fff", cursor:"pointer", fontFamily:FONT }}>Mégse</button>
            ...
            <div>
              {(mt.beveteliTetelek||[]).length===0
                ...
                : (mt.beveteliTetelek||[]).map((t,i)=>{
                    ...
                    return (
                      <div key={i} style={{ display:"flex", gap:12, padding:"7px 0", borderBottom:`1px solid ${C.bg}` }}>
...
export default function MunkatipusokPage({ userRole }) {
  ...
  return (
    <div style={{ padding:"24px 28px", fontFamily:FONT, maxWidth:900 }}>
      ...
      <div style={{ background:C.warningLight, border:`1px solid ${C.warningLight}`, borderRadius:10, padding:"10px 14px", marginBottom:16, fontSize:12, color:C.warning }}>
      ...
      {ujOpen && (
        <div style={{ background:C.accentLight, border:"2px solid #93C5FD", borderRadius:12, padding:"16px", marginTop:12 }}>
          ...
          <div style={{ display:"flex", gap:8 }}>
            ...
            <button onClick={()=>{setUjOpen(false);setUjNev("");}} style={{ padding:"8px 14px", border:`1.5px solid ${C.border}`, borderRadius:8, background:"#fff", cursor:"pointer", fontFamily:FONT }}>Mégse</button>
