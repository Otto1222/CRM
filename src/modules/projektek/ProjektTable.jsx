              ...
              return (
                <tr key={p.id} onClick={()=>onSelect(p)}
                  ...
                  {showPenz && <>
                    ...
                    <td style={{ padding:"11px 12px", fontWeight:700, color:(kalk?.haszon||0)>=0?C.success:C.danger }}>{kalk?.nettoBevitel>0?ft(kalk.haszon):"—"}</td>
                    <td style={{ padding:"11px 12px" }}>{kalk?.haszonPct!=null&&kalk?.nettoBevitel>0?<span style={{ background:(kalk.haszon||0)>=0?C.successLight:C.dangerLight,color:(kalk.haszon||0)>=0?C.success:C.dangerDark,padding:"2px 7px",borderRadius:20,fontWeight:700,fontSize:11 }}>{kalk.haszonPct}%</span>:"—"}</td>
                  </>}
