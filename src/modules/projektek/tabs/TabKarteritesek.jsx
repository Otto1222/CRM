...
function StatusBadge({ status }) {
  const cfg = {
    elfogadott:  { bg:"#F0FDF4", color:C.success, label:"✅ Elfogadott" },
    ...
    <div style={{ padding:"20px 0", fontFamily:FONT }}>
      ...
      {lista.length > 0 && (
        <div style={{ display:"flex", gap:12, marginBottom:20 }}>
          {[
            { label:"Elfogadott",  value: ft(elfOsszeg), color:C.success, bg:"#F0FDF4" },
      ...
      {lista.length === 0 ? (
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
          {lista.map(k => {
            ...
            return (
              <div key={k.id} style={{ background:"#fff", borderRadius:10, border:`1.5px solid ${st==="elfogadott"?"#86EFAC":st==="elutasitott"?C.dangerLight:C.border}`, padding:"14px 16px" }}>
                <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", gap:10 }}>
                  ...
                  <div style={{ display:"flex", flexDirection:"column", gap:5, flexShrink:0 }}>
                    {st !== "elfogadott"  && <button onClick={() => handleDontse(k.id,"elfogadott")}  style={{ display:"flex", alignItems:"center", gap:4, padding:"5px 10px", background:"#F0FDF4", color:C.success, border:"1.5px solid #86EFAC", borderRadius:7, cursor:"pointer", fontSize:11, fontWeight:700, fontFamily:FONT }}><CheckCircle2 size={12}/> Elfogad</button>}
                    {st !== "elutasitott" && <button onClick={() => handleDontse(k.id,"elutasitott")} style={{ display:"flex", alignItems:"center", gap:4, padding:"5px 10px", background:C.dangerLight, color:C.danger, border:`1.5px solid ${C.dangerLight}`, borderRadius:7, cursor:"pointer", fontSize:11, fontWeight:700, fontFamily:FONT }}><XCircle size={12}/> Elutasít</button>}
                    {st === "elfogadott"  && <button onClick={() => handleDontse(k.id,"fuggoben")}    style={{ padding:"5px 10px", background:C.warningLight, color:C.warning, border:`1.5px solid ${C.warningLight}`, borderRadius:7, cursor:"pointer", fontSize:11, fontWeight:700, fontFamily:FONT }}>↩ Vissza</button>}
                  ...
                  {tagok.length > 0 && (
                    <div>
                      ...
                      <select value={form.responsibleWorkerId} onChange={e => setF("responsibleWorkerId", e.target.value)}
                        style={{ width:"100%", padding:"9px 11px", border:`1.5px solid ${C.accentLight}`, borderRadius:8, fontSize:13, fontFamily:FONT, outline:"none", background:"#fff" }}>
