...
function MunkalapRiportSor({ m, projekt, onPrint }) {
  ...
  return (
    <div style={{ background:"#fff", border:`1px solid ${C.border}`, borderRadius:12, marginBottom:8, overflow:"hidden" }}>
      {/* Fejléc sor */}
        ...
        <div style={{ flex:1 }}>
          <div style={{ display:"flex", gap:10, alignItems:"center" }}>
            ...
            {p.motor === "D" && (
              <span
                title={p.warning}
                style={{ fontSize:10, background:C.warningLight, color:C.warning, padding:"2px 7px", borderRadius:20, fontWeight:700, cursor:"help" }}
              >
                Régi motor
              </span>
            )}
            {p.motor === "A" && (
              <span
                style={{ fontSize:10, background:C.successLight, color:C.success, padding:"2px 7px", borderRadius:20, fontWeight:700 }}
              >
                Szabályalapú
              </span>
            )}
      ...
      {open && (
        <div style={{ borderTop:`1px solid ${C.bg}`, padding:"12px 16px", background:C.bg }}>
...
export default function TabRiport({ projekt, munkalapok }) {
  ...
  return (
    <div style={{ paddingTop:16 }}>
      ...
      <div style={{ background:C.accentLight, border:`1.5px solid ${C.accentLight}`, borderRadius:14, padding:"18px 20px", marginBottom:20 }}>
