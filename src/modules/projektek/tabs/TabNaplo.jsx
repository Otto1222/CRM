  ...
  return (
    <div style={{ paddingTop:16 }}>
      ...
      <div style={{ background:"#fff", border:"1px solid #E2E8F0", borderRadius:12, padding:14, marginBottom:16, display:"flex", gap:10 }}>
        ...
        <button onClick={handleSend} disabled={!szoveg.trim()} style={{ padding:"0 16px", background:szoveg.trim()?C.accent:C.border, color:"#fff", border:"none", borderRadius:9, cursor:szoveg.trim()?"pointer":"not-allowed", display:"flex", alignItems:"center", gap:6, fontWeight:600, fontSize:13, fontFamily:FONT }}>
          <Send size={14}/> Küld
        </button>
      </div>
      ...
      {/* Napló lista */}
      {naplo.length === 0 ? (
        <p style={{ textAlign:"center", color:C.muted, padding:"40px 0" }}>Nincs napló bejegyzés</p>
      ) : (
        <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
          {naplo.map((e,i) => (
            <div key={e.id||i} style={{ background:"#fff", border:"1px solid #E2E8F0", borderRadius:10, padding:"12px 16px", display:"flex", gap:14 }}>
              <div style={{ width:36, height:36, borderRadius:"50%", background:C.accentLight, display:"flex", alignItems:"center", justifyContent:"center", fontSize:16, flexShrink:0 }}>
              ...
              <div style={{ flex:1 }}>
                <div style={{ display:"flex", gap:10, alignItems:"center", marginBottom:3 }}>
                  ...
                  <span style={{ fontSize:11, color:C.muted }}>{e.user || "Rendszer"}</span>
                  <span style={{ fontSize:11, color:C.border, marginLeft:"auto" }}>{new Date(e.datum).toLocaleString("hu-HU")}</span>
                </div>
                {e.reszletek && <p style={{ fontSize:13, color:C.muted, margin:0, lineHeight:1.5 }}>{e.reszletek}</p>}
              </div>
            </div>
          ))}
        </div>
