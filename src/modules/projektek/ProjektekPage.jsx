...
import { exportToExcel, exportToPDF, exportToCSV } from "../../lib/exportService.js";
...
export default function ProjektekPage({ data, currentUser, onNavigateMunkalap }) {
  ...
  useEffect(() => {
    function refresh(e) {
      if (!e.detail?.collection || e.detail.collection==="projektek" || e.detail.collection==="all") {
        ...
        // Ha éppen egy projektet nézünk, azt is frissítjük
        if (sel) {
  ...
  const filtered = projektek.filter(p => {
    ...
    const matchQ = !q || p.nev?.toLowerCase().includes(q2) || p.projektkod?.toLowerCase().includes(q2) || p.clientNev?.toLowerCase().includes(q2);
  ...
  return (
    <div style={{ padding:"24px 28px", fontFamily:FONT }}>
      ...
      <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", marginBottom:20 }}>
        <div>
          <h1 style={{ fontFamily:FONT_HEADING, fontSize:24, fontWeight:800, color:"#0F172A", margin:"0 0 4px" }}>🏗️ Projektek</h1>
        ...
        <div style={{ display:"flex", gap:8, alignItems:"center" }}>
          ...
          <button onClick={()=>setUjOpen(true)} style={{ display:"flex", alignItems:"center", gap:7, padding:"9px 18px", background:"#2563EB", color:"#fff", border:"none", borderRadius:10, cursor:"pointer", fontWeight:700, fontSize:14, fontFamily:FONT }}>
      ...
      <div style={{ position:"relative", marginBottom:12 }}>
        ...
        <input value={q} onChange={e=>setQ(e.target.value)} placeholder="Keresés név, kód, ügyfél szerint…"
      ...
      {/* Státusz szűrő (görgethető) */}
      <div style={{ display:"flex", gap:6, overflowX:"auto", paddingBottom:8, marginBottom:16 }}>
        {SZUROK.map(s=>(
          <button key={s} onClick={()=>setTabFilter(s)}
            style={{ padding:"5px 12px", borderRadius:20, border:"none", cursor:"pointer", fontFamily:FONT, fontWeight:600, fontSize:12, whiteSpace:"nowrap",
      ...
      {/* Táblázat */}
      <ProjektTable projektek={filtered} munkalapok={munkalapok} onSelect={setSel} userRole={userRole}/>
      ...
      {ujOpen && (
        <ProjektForm
          ...
          onSaved={p=>{ setProjektek(loadProjektek()); setSel(p); }}
        />
      )}
    </div>
  );
