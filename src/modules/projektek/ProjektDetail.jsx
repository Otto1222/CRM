...
export default function ProjektDetail({ projekt, munkalapok, onBack, onNavigateMunkalap, currentUser }) {
  ...
  const tabContent = () => {
    ...
    switch(tab) {
      case "attekintes":   return <TabAttekintes    {...props}/>;
      case "ajanlatok":    return <TabAjanlatok     {...props}/>;
      case "munkalapok":   return <TabMunkalapok    {...props} onNavigate={onNavigateMunkalap}/>;
      case "koltsegek":    return <TabKoltsegek     {...props}/>;
      case "dokumentumok": return <TabDokumentumok  {...props}/>;
      case "utemezas":     return <TabUtemezas      {...props}/>;
  ...
  return (
    <div style={{ padding:"24px 28px", fontFamily:FONT, maxWidth:1100 }}>
      ...
      <div style={{ display:"flex", alignItems:"flex-start", gap:16, marginBottom:20 }}>
        <button onClick={onBack} style={{ border:"none", background:"none", cursor:"pointer", color:"#64748B", padding:"4px", display:"flex", alignItems:"center", gap:6, fontSize:13, fontFamily:FONT }}>
          <ArrowLeft size={18}/> Vissza
        </button>
        <div style={{ flex:1 }}>
          <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:4 }}>
            <span style={{ fontSize:12, fontWeight:700, color:"#64748B" }}>{lokalProjekt.projektkod}</span>
            {lokalProjekt.kulsoAzonosito && <span style={{ fontSize:11, color:"#94A3B8" }}>/ {lokalProjekt.kulsoAzonosito}</span>}
            <span style={{ background:stCfg.szin, color:"#fff", borderRadius:20, padding:"3px 12px", fontSize:12, fontWeight:700 }}>{lokalProjekt.status}</span>
          </div>
          <h1 style={{ fontFamily:FONT_HEADING, fontSize:22, fontWeight:800, color:"#0F172A", margin:"0 0 2px" }}>{lokalProjekt.nev}</h1>
          <p style={{ fontSize:13, color:"#64748B", margin:0 }}>
            {lokalProjekt.clientNev} · {lokalProjekt.csapatNev || "—"} · {lokalProjekt.tipus}
          </p>
        </div>
        <div style={{ display:"flex", gap:8 }}>
          <button onClick={handlePrint} style={{ display:"flex", alignItems:"center", gap:6, padding:"8px 14px", background:"#F1F5F9", color:"#475569", border:"none", borderRadius:9, cursor:"pointer", fontWeight:600, fontSize:13, fontFamily:FONT }}>
            <Printer size={14}/> PDF
          </button>
          <button onClick={()=>setEditOpen(true)} style={{ display:"flex", alignItems:"center", gap:6, padding:"8px 16px", background:"#2563EB", color:"#fff", border:"none", borderRadius:9, cursor:"pointer", fontWeight:600, fontSize:13, fontFamily:FONT }}>
      ...
      <div style={{ display:"flex", gap:2, borderBottom:"2px solid #E2E8F0", marginBottom:0, overflowX:"auto" }}>
        {TABS.map(t=>(
          <button key={t.id} onClick={()=>setTab(t.id)}
            style={{ padding:"10px 14px", border:"none", background:"none", cursor:"pointer", fontFamily:FONT,
